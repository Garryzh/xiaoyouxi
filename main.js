/* ============================================================================
   ...
   Tuned to match the Open Sea reference look: low camera, soft Gerstner swell,
   FBM micro-surface, DAY↔DUSK spectral sky, sparse crest foam, sun glitter.
   ============================================================================ */
...
const WAVES = WAVE_DEFS.map(({ dir, wavelength, steepness }) => {
  ...
  return {
    ...
    k: float(k),
    c: float(Math.sqrt(9.8 * k)), // matches reference phase term in f = k·(dot − c·t)
...
/* ---------------------------------------------------------------------------
   Ocean — lighting built manually in a TSL color node
   --------------------------------------------------------------------------- */
const oceanColor = Fn(() => {
  ...
  const fresnel = float(0.02).add(float(0.98).mul(pow(float(1.0).sub(max(dot(N, V), 0.0)), 5.0)));
...
function setupScene() {
  ...
  // r178: PostProcessing · r183+: RenderPipeline (same API)
  const Pipeline = THREE.PostProcessing || THREE.RenderPipeline;
...
async function frame() {
  ...
  if (typeof postProcessing.renderAsync === 'function') {
  ...
  if (fpsTime >= 0.5) {
    ...
    // Soft performance protection: drop DPR if sustained low FPS
    if (fps < 26) {
      lowFpsMs += 500;
      if (lowFpsMs > 2000) {
        const next = Math.max(1, renderer.getPixelRatio() - 0.25);
        if (next < renderer.getPixelRatio() - 0.01) {
      ...
      lowFpsMs = Math.max(0, lowFpsMs - 250);
    }
  }
