...
function initThree() {
  ...
  canvas.addEventListener('webglcontextlost', (e) => {
    ...
    showFatal('WEBGL CONTEXT LOST',
      'The GPU context was lost — rendering has stopped. Press RETRY to reinitialize, or LOWER QUALITY to reduce the render load first.');
  });
...
// Position the main loop: cinematic â†’ cineTime % 176; first hand-off from the
// intro sting outside cinematic â†’ 0; otherwise keep the paused position.
...
window.addEventListener('keydown', (e) => {
  ...
  if (k >= '1' && k <= '4') flyToPreset(PRESET_KEYS[+k - 1]);
    ...
    const def = PARAM_DEFS.find((d) => d.key === 'debug');
    if (!def) return;
    P.debug = +k;
    urlOverrideKeys.delete('debug');
    applyParam(def);
    saveStorage();
    toast('DEBUG VIEW — ' + k);
  }
  else if (k === 'c') toggleCine();
  else if (k === 'r' || k === 'o') toggleOrbit();
  else if (k === 'q') {
    const idx = TIER_ORDER.indexOf(tierName);
    setTier(TIER_ORDER[(idx + 1) % TIER_ORDER.length]);
  }
